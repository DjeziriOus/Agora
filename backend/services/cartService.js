 ...
 */
export const addItem = async (userId, productId, quantity = 1, variantId = null) => {
  ...
  if (normalizedVariantId) {
    const variant = Array.isArray(product.variants)
      ? product.variants.find((v) => normalizeVariantId(v.code) === normalizedVariantId)
      : null;
  ...
  if (existingItem) {
    ...
    cart.items.push({ productId, quantity, variantId: normalizedVariantId, selected: true });
  }
 ...
 */
export const updateQuantity = async (userId, productId, quantity, variantId = null) => {
  const normalizedVariantId = normalizeVariantId(variantId);

...
const updateShop = async ({ shopId, ownerId, updateData, files = {} }) => {
	...
	if (updateData.contactEmail !== undefined) shop.contactEmail = updateData.contactEmail;
	if (updateData.contactPhone !== undefined) shop.contactPhone = updateData.contactPhone;
	if (updateData.contactAddress !== undefined) shop.contactAddress = updateData.contactAddress;

	// Replace logo if a new file was uploaded.
	const newLogo = await replaceShopImage(files.logo?.[0], shop.logo, "shopLogo");
	if (newLogo) shop.logo = newLogo;

	// Replace banner if a new file was uploaded.
	const newBanner = await replaceShopImage(files.banner?.[0], shop.banner, "shopBanner");

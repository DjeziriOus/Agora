    ...
    return (
      <div className="min-h-screen bg-[var(--agora-bg)] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/catalogue" className="text-[var(--agora-primary)] hover:underline">
            Retour au catalogue
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--agora-bg)]">
      <div className="relative h-48 sm:h-64 md:h-80 bg-gradient-to-r from-[var(--agora-primary)] to-[#7986CB]">
        {store.banner && (
          <Image src={store.banner} alt={`${store.name} banner`} fill className="object-cover" priority />
        )}
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end gap-4 sm:gap-6">
              ...
              <div className="flex-1 text-white pb-1">
                <h1 className="font-display text-2xl sm:text-3xl font-bold mb-2">{store.name}</h1>
                <p className="text-white/80 text-sm sm:text-base line-clamp-2 max-w-2xl">{store.description}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[var(--agora-mid)]">{store.productCount || 0} produits</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[var(--agora-mid)]" />
              <span className="text-sm text-[var(--agora-mid)]">Membre depuis {memberSince}</span>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {products.length > 0 ? (
          ...
          <EmptyState type="products" title="Aucun produit" description="Cette boutique n'a pas encore de produits." />
        )}
      </div>
    </div>
  );
